...
class AsioExecutor {
 public:
  ...
  void RegisterSvrStartFunc(std::function<void()>&& start_func) {
    if (omnirt_unlikely(state_.load() != State::kPreStart))
   ...
   */
  void Start() {
    if (omnirt_unlikely(std::atomic_exchange(&state_, State::kStart) != State::kPreStart))
      throw std::runtime_error("Method can only be called when state is 'PreStart'.");
   ...
   */
  void Shutdown() {
    if (omnirt_unlikely(std::atomic_exchange(&state_, State::kShutdown) == State::kShutdown))
      return;
