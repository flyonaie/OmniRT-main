 ...
 public:
    ...
    struct source_location {
        ...
        constexpr source_location(
            ...
            const char* function = __FUNCTION__) noexcept
            : file_(file), line_(line), function_(function) {}

        /**
         * @brief 获取当前源码位置信息
         * @return 返回包含当前位置信息的source_location对象
         */
        ...
        /**
         * @brief 获取列号
         * @details 由于不同编译器对列号支持不一致，这里统一返回0
         * @return 固定返回0
         */
...
class SimpleLogger {
 public:
  ...
  static void Log(uint32_t lvl,
    ...
    if (omnirt_unlikely(lvl >= kLvlNameArraySize))
...
#define AIMRT_HANDLE_LOG(__lgr__, __lvl__, __fmt__, ...)                                 \
  do {                                                                                   \
    ...
    if (__lvl__ >= __cur_lgr__.GetLogLevel()) {                                          \
      ...
      const auto __location__ = aimrt_src_loc::source_location::current(                  \
          __FILE__, __LINE__, __FUNCTION__);                                             \
      __cur_lgr__.Log(                                                                   \
          __lvl__, __location__.line(), __location__.column(), __location__.file_name(), \
          __location__.function_name(), __log_str__.c_str(), __log_str__.size());        \
    }                                                                                    \
  } while (0)
