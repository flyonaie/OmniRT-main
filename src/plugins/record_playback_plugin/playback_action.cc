...
void PlaybackAction::Initialize(YAML::Node options_node) {
  ...
  if (!options_.topic_meta_list.empty()) {
    ...
    for (auto item : options_.topic_meta_list) {
      ...
      if (omnirt_unlikely(finditr == metadata_.topics.end())) {
...
bool PlaybackAction::StartSignalPlayback(uint64_t skip_duration_s, uint64_t play_duration_s) {
  ...
  if (omnirt_unlikely(options_.mode != Options::Mode::kSignal)) {
...
bool PlaybackAction::OpenNewDb() {
  ...
  if (omnirt_unlikely(cur_db_file_index_ >= metadata_.files.size())) {
    return false;
  }
  ...
  if (omnirt_unlikely(stop_playback_timestamp_ && cur_db_start_timestamp > stop_playback_timestamp_)) {
    return false;
  }
...
void PlaybackAction::AddPlaybackTasks(const std::shared_ptr<void>& task_counter_ptr) {
  {
    std::lock_guard<std::mutex> lck(playback_state_mutex_);
    if (omnirt_unlikely(playback_state_ == PlayBackState::kGetStopSignal)) {
      return;
    }
  }
  ...
  if (omnirt_unlikely(db_ == nullptr)) {
    // first record
    if (!OpenNewDb()) {
      std::lock_guard<std::mutex> lck(playback_state_mutex_);
      playback_state_ = PlayBackState::kGetStopSignal;
  ...
  while (true) {
    ...
    if (ret == SQLITE_ROW) {
      ...
      if (omnirt_unlikely(stop_playback_timestamp_ && timestamp >= stop_playback_timestamp_)) {
        std::lock_guard<std::mutex> lck(playback_state_mutex_);
        playback_state_ = PlayBackState::kGetStopSignal;
        break;
      }
      ...
      if (omnirt_unlikely(cur_start_timestamp == 0)) {
        cur_start_timestamp = timestamp;
      } else if (omnirt_unlikely((timestamp - cur_start_timestamp) >= 1000000000 || records.size() >= kMaxRecordSize)) {
        break;
      }
    } else {
      if (!OpenNewDb()) {
  ...
  if (omnirt_unlikely(records.empty())) {
    std::lock_guard<std::mutex> lck(playback_state_mutex_);
    playback_state_ = PlayBackState::kGetStopSignal;
    return;
  }
