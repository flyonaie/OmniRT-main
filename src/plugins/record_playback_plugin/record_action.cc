...
bool RecordAction::StartSignalRecord(uint64_t preparation_duration_s, uint64_t record_duration_s) {
  ...
  if (omnirt_unlikely(recording_flag_)) {
...
void RecordAction::AddRecordImpl(OneRecord&& record) {
  ...
  } else if (omnirt_unlikely(cur_data_size_ * estimated_overhead_ >= max_bag_size_)) {
  ...
  if (omnirt_unlikely(cur_exec_count_ == 0)) {
  ...
  if (omnirt_unlikely(cur_exec_count_ >= 1000)) {
    cur_exec_count_ = 0;
