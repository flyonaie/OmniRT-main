        ...
        service_invoke_wrapper_ptr->callback =
            [this,
             ...
             req_id_buf](aimrt::rpc::Status status) {
              ...
              if (omnirt_unlikely(mqtt_pkg_size > max_pkg_size_)) {
...
void MqttRpcBackend::Invoke(
  ...
  try {
    ...
    if (omnirt_unlikely(serialization_type.size() > 255)) {
    ...
    if (omnirt_unlikely(mqtt_sub_topic.size() > 255)) {
      client_invoke_wrapper_ptr->callback(aimrt::rpc::Status(AIMRT_RPC_STATUS_CLI_UNKNOWN));
      return;
    }
    ...
    if (omnirt_unlikely(keys.size() > 255)) {
