...
#include "aimrt_module_c_interface/llm/llm_handle_base.h"
...
typedef struct {
  ...
  /**
   * @brief Function to get llm handle
   * @note
   * Parameter definition:
   * Input 1: Pointer to impl
   * Output: Llm handle
   */
  const aimrt_llm_handle_base_t* (*llm_handle)(void* impl);

  /**
   * @brief Function to get module manager handle
   * @note
   * Parameter definition:
   * Input 1: Pointer to impl
   * Output: Module manager handle
   */
  const aimrt_module_manager_base_t* (*module_manager)(void* impl);
