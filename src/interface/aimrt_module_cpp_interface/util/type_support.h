...
class TypeSupportRef {
 public:
  ...
  std::vector<aimrt_string_view_t> GetSerializationTypesSupportedList() const {
    const auto* list = base_ptr_->serialization_types_supported_list(base_ptr_->impl);
    const auto size = base_ptr_->serialization_types_supported_num(base_ptr_->impl);
    return std::vector<aimrt_string_view_t>(list, list + size);
  }
  ...
  bool CheckSerializationTypeSupported(std::string_view serialization_type) const {
    auto supported_types = GetSerializationTypesSupportedList();
    return std::find_if(
        supported_types.begin(), supported_types.end(),
        [serialization_type](const aimrt_string_view_t& s) {
