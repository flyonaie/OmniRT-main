...
void ParameterModule::FuncSetString(uint32_t count)
{
  std::string key = "key-string-" + std::to_string(count);
  std::string val = "val-string-" + std::to_string(count);
  parameter_handle_.SetParameter(key, val);
  AIMRT_INFO("Set parameter, key: '{}', val: '{}'", key, val);
}
void ParameterModule::FuncSetInt(uint32_t count)
{
  std::string key = "key-int-" + std::to_string(count);
  int val = 123;
  parameter_handle_.SetParameter(key, std::to_string(val));
  AIMRT_INFO("Set parameter, key: '{}', val: '{}'", key, val);
}
typedef struct {
  int a;
...
void ParameterModule::FuncSetStruct(uint32_t count)
{
  std::string key = "key-struct-" + std::to_string(count);
  parameter_struct_t val = {123, 456, 789};
  parameter_handle_.SetParameter(key, std::to_string(val.a) + "," + std::to_string(val.b) + "," + std::to_string(val.c));
  AIMRT_INFO("Set parameter, key: '{}', val: '{}'", key, val);
}
