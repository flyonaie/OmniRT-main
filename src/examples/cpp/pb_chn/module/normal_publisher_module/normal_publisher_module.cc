...
bool NormalPublisherModule::Initialize(aimrt::CoreRef core) {
  ...
  try {
    // Read cfg
    auto file_path = core_ref.GetConfigurator().GetConfigFilePath();
    AIMRT_INFO("----------------file_path: {}", file_path);
    if (!file_path.empty()) {
    ...
    // Get executor handle
    executor_ = core_ref.GetExecutorManager().GetExecutor("work_thread_pool");
    AIMRT_CHECK_ERROR_THROW(executor_ && executor_.SupportTimerSchedule(),
                            "Get executor 'work_thread_pool' failed.");

    // Register publish type
    publisher_ = core_ref.GetChannelHandle().GetPublisher(topic_name_);
    AIMRT_CHECK_ERROR_THROW(publisher_, "Get publisher for topic '{}' failed.", topic_name_);
    bool ret = publisher_.RegisterPublishType(GetProtobufMessageTypeSupport<aimrt::protocols::example::ExampleEventMsg>());
...
void NormalPublisherModule::MainLoop() {
  try {
    ...
    while (run_flag_) {
      ...
      // publisher_.Publish(std::string_view msg_type, ContextRef ctx_ref, const void *msg_ptr)
    }
