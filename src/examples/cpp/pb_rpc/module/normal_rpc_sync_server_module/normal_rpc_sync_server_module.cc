...
bool NormalRpcSyncServerModule::Initialize(aimrt::CoreRef core) {
  ...
  SetLogger(core_ref.GetLogger());

  try {
    // Read cfg
    std::string file_path = std::string(core_ref.GetConfigurator().GetConfigFilePath());
    ...
    if (service_name_.empty()) {
      ret = core_ref.GetRpcHandle().RegisterService(service_ptr_.get());
    } else {
      ret = core_ref.GetRpcHandle().RegisterService(service_name_, service_ptr_.get());
    }
