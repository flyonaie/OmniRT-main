...
class RpcHandleProxy {
 ...
 private:
  ...
  void MergeServerContextToClientContext(
    ...
    if (omnirt_unlikely(server_ctx_ref.GetType() != aimrt_rpc_context_type_t::AIMRT_RPC_SERVER_CONTEXT ||
        client_ctx_ref.GetType() != aimrt_rpc_context_type_t::AIMRT_RPC_CLIENT_CONTEXT)) {
      // TODO warn log
      return;
    }
