...
#include "core/module/module_proxy.h"
...
class ModuleManager {
 ...
 public:
  ModuleManager()
      : logger_ptr_(std::make_shared<aimrt::common::util::LoggerWrapper>()) {
        std::cout << "ModuleManager--get_log_level_func: " << logger_ptr_->get_log_level_func() << std::endl;
       }
  ...
  const ModuleManagerProxy& GetModuleManagerProxy(const util::ModuleDetailInfo& module_info);
  const ModuleManagerProxy& GetModuleManagerProxy(std::string_view module_name = "core") {
    return GetModuleManagerProxy(
 ...
 private:
  ...
  void* GetModule(const std::string& module_name);
 ...
 private:
  ...
  // 模块管理器代理指针
  std::shared_ptr<ModuleManagerProxy> module_manager_proxy_ptr;

  // 模块管理器回调
  ModuleManagerCallbacks module_manager_callbacks;
};
