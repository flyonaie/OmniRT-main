...
void ExecutorManager::Initialize(YAML::Node options_node) {
  ...
  std::cout << "ExecutorManager--options_node: " << (options_node) << std::endl;
  AIMRT_INFO("options_node valid: {}, options_node is null: {}", 
             options_node ? "true" : "false", 
             options_node.IsNull() ? "true" : "false");
  bool flag = static_cast<bool>(options_node);
  std::cout << "flag: " << flag << std::endl;
  if (options_node && !options_node.IsNull())
  {
      AIMRT_INFO("options_node: flag {}", flag);
      options_ = options_node.as<Options>();
  }

  // 生成executor
  int executor_count = options_.executors_options.size();
  AIMRT_INFO("options_node: executor_count {}", executor_count);
  for (auto& executor_options : options_.executors_options) {
    AIMRT_INFO("1111111111");
  
    // 检查executor_options.name是否重复，没有则创建
    AIMRT_CHECK_ERROR_THROW(
    ...
    // 执行开头注册的函数生成executor,finditr->second和finditr->second()是不一样的
  ...
  AIMRT_INFO("options_node: executor_vec__count {}", executor_vec_.size());
...
/**
 * @brief 注册NativeExecutor生成函数。
 */
 void ExecutorManager::RegisterNativeExecutorGenFunc() {
  RegisterExecutorGenFunc("native_thread", [this]() -> std::unique_ptr<ExecutorBase> {
    auto ptr = std::make_unique<NativeThreadExecutor>();
...
/**
 * @brief 注册TBoxExecutor生成函数。
 */
/*void ExecutorManager::RegisterTBoxExecutorGenFunc() {
  RegisterExecutorGenFunc("tbox", []() {
    return std::make_unique<TBoxExecutor>();
  });
}*/
