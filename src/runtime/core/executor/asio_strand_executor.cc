...
void AsioStrandExecutor::Initialize(std::string_view name,
  ...
  strand_ptr_ = std::make_unique<Strand>(io_ptr->get_executor());
...
void AsioStrandExecutor::ExecuteAt(
  ...
  try {
    auto& io_context = static_cast<asio::io_context&>(strand_ptr_->context());
    auto timer_ptr = std::make_shared<asio::system_timer>(io_context);
    timer_ptr->expires_at(tp);
    timer_ptr->async_wait([this, timer_ptr,
                           task = std::move(task)](asio::error_code ec) mutable {
